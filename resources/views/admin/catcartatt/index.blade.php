...
<script>
    $(document).ready(function() {
        ...
        $('.myform').submit(function() {
            if (confirm('هل ان متأكد من حذف البضاعه الحاليه؟')) {
    ...
    <div class="content-wrapper">
        <div class="content-header row">
            <div class="content-header-left col-md-8 col-12 mb-2">
                <center>
                <h3 class="content-header-title font-theme ls-0">تقارير البضاعه الحاليه
                </h3>
                </center>
            </div>
{{--            <div class="content-header-right col-md-4 col-12 mb-2">--}}
{{--                <a href="{{url('admin/catcart/create')}}" class="btn btn-primary" ><i class="icon-plus"></i> إضافة </a>--}}
{{--            </div>--}}
{{--            <div class="content-header-right col-md-4 col-12 mb-2">--}}
                ...
                <div class="card-block">
                    <div class="card-body">
                        <div class="table-responsive">
                            <table class="table table-head-fixed text-nowrap table-striped-td table-sm-td text-center datatable" >
                                <thead>
                                <tr>
                                    <th>اسم الصنف</th>
                                    <th>رقم الصنف</th>
                                ...
                                <tr>
                                    <td> <a href="{{(route('catcart.show',$drive->id))}}">{{$drive->cart->name ?? ''}}</a></td>
                                    <td>{{$drive->cart->number ?? ''}}</td>
                                    <td>{{$drive->cart->category ?? ''}}</td>
                                    <td>{{$drive->qty ?? ''}}</td>
                                    <td>{{$drive->sell_price ?? ''}}</td>
                                    <td>{{$drive->purchase_price ?? ''}}</td>
                                    <td>{{$drive->tax ?? ''}}</td>
                                    <td>{{$drive->unit ?? ''}}</td>

                                    <td>{{$drive->cart->suppler->name ?? ''}}</td>
                                    <td>{{ isset($drive->created_at) ? $drive->created_at->diffForHumans() :''	 }}</td>
                                    <td>

                                        <form class="myform"  id="{{$drive->id}}"  method="post"  action="{{'catcartattribute/delete/'.$drive->id}}">
                                            @csrf
...
{{--                </button>--}}
{{--            </div>--}}
{{--            <form id="subscription_form" action="{{url('admin/getPurshaseData')}}"  novalidate method="GET">--}}
{{--                <div class="">--}}
