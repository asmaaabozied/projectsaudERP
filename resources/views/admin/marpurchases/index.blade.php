...
<script>
    $(document).ready(function() {
        ...
        $('.myform').submit(function() {
            if (confirm('هل ان متأكد من حذف استلام بضاغه المشتريات؟')) {
    ...
    <div class="content-wrapper">
        <div class="content-header row">
            <div class="content-header-left col-md-8 col-12 mb-2">
                <center>
                <h3 class="content-header-title font-theme ls-0">استلام بضاغه المشتريات
                </h3>
                </center>
            </div>
            <div class="content-header-right col-md-4 col-12 mb-2">
                <a href="{{url('admin/marpurchases/create')}}" class="btn btn-primary" ><i class="icon-plus"></i> إضافة </a>
            </div>
{{--            <div class="content-header-right col-md-4 col-12 mb-2">--}}
                    ...
                    <div class="card-body">
                        <div class="table-responsive">
                            <table class="table table-head-fixed text-nowrap table-striped-td table-sm-td text-center datatable" >
                                ...
                                <tr>
                                    <td> <a>{{$drive->store->name ?? ''}}</a></td>
                                    <td>{{$drive->number1}}</td>
                                    <td>{{$drive->date}}</td>

                                    <td>{{$drive->suppler->name ?? ''}}</td>
                                    <td>{{ isset($drive->created_at) ? $drive->created_at->diffForHumans() :''	 }}</td>
                                    <td>

                                        <form class="myform"  id="{{$drive->id}}"  method="post"  action="{{'marpurchases/delete/'.$drive->id}}">
                                            @csrf
