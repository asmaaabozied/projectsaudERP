...
//        }
        ...
        $stores = Store::get();
        $suppliers = Suppliers::get();
        $drivers = Purchase::where('company_id',$user->company_id)->where('return', '0')->get();
...
//        }
        $user = User::find(Auth::id());
        $drivers = Purchase::where('return', '0');
        if ($request->client) {
...
//        }
        ...
        $drivers = Purchase::where('company_id',$user->company_id)->get();
...
//        $products=Products::all();
    ...
    public function prints($id)
    {
        ...
        $pdf = \PDF::loadView('admin.purchase.print', compact('products','driver'));
...
//        $products = PurchaseProduct::where('purchase_id', $id)->get();
//
//        $pdf = PDF::loadView('admin.purchase.show', compact('driver', 'suppliers', 'stores', 'classifications', 'products'));
